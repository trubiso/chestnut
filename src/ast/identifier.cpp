...
bool RichIdentifier::has_unreached() const {
	return std::any_of(path_.cbegin(), path_.cend(), [](Segment const& segment) { return segment.is_unreached(); });
}

RichIdentifier::Segment& RichIdentifier::last_unreached_segment() {
...
RichIdentifier::RichIdentifier(Spanned<std::string> name) : absolute_ {false}, path_ {} {
	path_.emplace_back(std::move(name.value), std::nullopt, std::nullopt, std::move(name.span));
}

RichIdentifier::RichIdentifier(bool absolute, std::vector<Segment> path)
	: absolute_ {absolute}
	, path_ {std::move(path)} {
...
std::ostream& operator<<(std::ostream& os, RichIdentifier::Segment const& segment) {
	os << segment.name;
...
std::ostream& operator<<(std::ostream& os, RichIdentifier const& identifier) {
